...
// Authority::Authority() : Person(id, pw, name)
// {
//     this->uname = name;
// }
...
bool Authority::authenticate()
{
    ...
    bool found = true;
    // ifstream inputObj;
    // inputObj.open("authority_auth_db.txt");
    // if (inputObj.fail())
    // {
    //     cout << "\nERROR: Authentication file couldn't be opened!\n\n";
    //     exit(0);
    // }
    // // read each record and check
    // inputObj >> id >> password;
    // while (!inputObj.eof())
    // {
...
sedr:
    system("cls");
    // name this as extra_diet rate then in this update(add ,remove update ratelist) and check ka options
    cout << "\n1.Check List";
    cout << "\n2.Update List";
    cout << "\n3.exit";
    cout << "\nEnter here: ";
...
void Authority::check_extra_diet_rate() // should be in parent class and available to both
{
    // read file and just show it to user;
...
void Authority::set_diet_rate()
{
    ...
    //  normal diet rate
    // only authority can set diet rate
}
void Authority::set_notification()
{
    ...
    if (fout.is_open())
    {
        ...
        cout << "\nerror\n";
    // setting some message like today mess is off or notifications like
    //  timing of mess  has cahnges //extra jalebi something like this
}
