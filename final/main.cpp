...
Student *authenticate(string id, string password, string filename)
{
    ...
    if (fin.is_open())
    {
        ...
        while (i < Num)
        {
            ...
            fin >> s; // date getting filled from file
...
void authority_login()
{
    ...
    cout << "\nEnter password : ";
    password = takePasswdFromUser();
    // Authority obj(id, password, "Neeraj");
    Authority obj;
...
int main()
{
    ...
    // Student s[Num];
    // ofstream fout;
    // fout.open("student_record.csv", ios::app);
    // if (fout.is_open())
    // {
    //     for (int i = 0; i < Num; i++)
    //     {
    //         cout << "\nset details of student" << i + 1 << endl;
    //         s[i].setDetails();
    //         fout << s[i]; // inserting details in file;
    //     }
    //     fout.close();
    // }
start:
