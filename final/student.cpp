...
{
    // yha aakr check kro mess off hai ya nhi if yes then -000 kro
    int day;
    int meal;
    cout << "Enter day \n";
    cin >> day;
    ...
    if (fin.is_open())
    {
        ...
        while (i < Num) // reading line by line;
        {
            ...
            {
                // u can make insertion operator overload for vector
                vect[day][meal] = '1';
            }
            int n = vect.size();
            for (int i = 0; i < n - 1; i++)
            ...
            fout << vect.at(n - 1) << '\n';
            // fout << vect; // writing to file******************errror
            vect.clear();
            i++;
        }
        fin.close();
...
{
    // cout << "\nreturning bill details\n";
    ifstream fin; // only reading
    fin.open("mess_record.csv", ios::in);
    if (fin.is_open())
    {
        int i = 0;
        ...
        {
            vector<string> vect;
            fin >> vect; // reading each line;
            // if roll number gets matched;
            if (vect[0] == this->rollNum)
            {
        ...
        // cout << this->uname << "\n: your Total billl of " << day << " days : " << total << endl;
        fin.close();
        return total;
    }
    // read file mess_record and do the total and return
    //  returns bill of current month and gives option to see previos month bills
}
...
void Student::check_balance()
{
    ...
    // return how much amount is left
}
void Student::show_extra_diet()
{
    cout << "\nreturning extra diet balances\n";
    // shows extra items that we bought
}
...
void Student::check_diet_rate()
{
    ...
    // for checking list rate for diet and extra items
}
void Student::show_notification()
{
    // // read file named notifications
    ...
    // for checking various notifications
}
